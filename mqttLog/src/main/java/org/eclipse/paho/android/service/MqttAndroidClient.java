...
//	}
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (MqttServiceConstants.CONNECT_ACTION.equals(action)) {
			...
			Log.i(LogTagConstant.TAG, "MqttAndroidClient广播onReceive收到消息：" + data);
		}
		...
		if (callback != null) {
			...
			try {
				if (messageAck == Ack.AUTO_ACK) {
					...
					Log.i(LogTagConstant.TAG, "messageArrivedAction callback.messageArrived回调给MainActivity：" + message);
