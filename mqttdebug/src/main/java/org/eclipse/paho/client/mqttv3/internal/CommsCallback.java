...
import org.eclipse.paho.android.service.LogTagConstant;
...
import org.eclipse.paho.client.mqttv3.QueuedThreads;
...
public class CommsCallback implements Runnable {
				...
				if (running) {
					...
					synchronized (messageQueue) {
					    if (!messageQueue.isEmpty()) {
							...
							Log.e(LogTagConstant.BUFFER_VECTOR_TAG, "==messageQueue.size()===" + messageQueue.size());
							messageQueue.removeElementAt(0);
							Log.e(LogTagConstant.BUFFER_VECTOR_TAG, "==after remove messageQueue.size()===" + messageQueue.size());
					    }
					}
					if (null != message) {
	...
	public void messageArrived(MqttPublish sendMessage) {
		try {
			Log.i(LogTagConstant.BUFFER_TAG, "5=== CommsCallback messageArrived" + sendMessage.getPayload().toString());
			...
			synchronized (spaceAvailable) {
				while (running && !quiescing && messageQueue.size() >= INBOUND_QUEUE_SIZE) {
					try {
						...
						// TODO: 2018/5/16 change here
//						spaceAvailable.wait(200);
						spaceAvailable.wait(10);
						Log.e(LogTagConstant.BUFFER_VECTOR_TAG, "========= CommsCallback messageArrived 等待200ms========" +"messageQueue.size()====" + messageQueue.size());
					} catch (InterruptedException ex) {
					}
				}
			}
	...
	private void handleMessage(MqttPublish publishMessage)
		...
		Log.i(LogTagConstant.BUFFER_TAG, "6=== CommsCallback handleMessage" + publishMessage.getPayload().toString());
...
//		Log.i(LogTagConstant.BUFFER_TAG, "7=== CommsCallback deliverMessage" + aMessage.toString());
...
//				LoggerPrintInstance.getLogger().i(LogTagConstant.PRINT_LOG, "0=== MqttCallback messageArrived"+ aMessage.toString());
//				delivered = true;
//			}
//		}
//
...
//			aMessage.setId(messageId);
//			LoggerPrintInstance.getLogger().i(LogTagConstant.PRINT_LOG, "1=== MqttCallback messageArrived"+ aMessage.toString());
...
//	}
	...
	// TODO: 2018/5/17 修改后
	protected boolean deliverMessage(String topicName, int messageId, MqttMessage aMessage) throws Exception {
		QueuedThreads.getInstance().push(aMessage.toString());
		return true;
	}
